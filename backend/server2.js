...
app.post('/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        const rows = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName], {
            header: 1,
            defval: null // preserve empty cells as null
        });
        ...
        for (const row of excelData) {
            ...
            const query = `
                ...
                (s_id, sname, DOB, s_father, phone_no, address, semester, department, aadhaar, email) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
            ...
            pool.query(query, [
                ...
                if (err) {
                    ...
                    pool.query('INSERT INTO attendance (s_id) VALUES (?)', [row["pin"]], (err2, result2) => {
                        if (err2) {
                            console.log('Error adding into Attendance:', err2);
                        } else {
                            console.log('Attendance initialized for:', row["pin"]);
                        }
                    });
                }
            });
